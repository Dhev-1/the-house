...
**home/ — done.** `hyprland.conf`: the autogenerated header, wiki-link lines,
stock example blocks (permissions, smart gaps) and restatement comments are
gone; every house-voice comment is untouched. `binds.conf`: the thirty repeated
`# NOTE: code:N = key N` trailers (the header above them says it once), garbled
leftovers (`#hBrave Browsr`, `#ROUPING…`, `#ADDED`), a mislabelled
`(UserScripts)` header and a dead commented bind are gone, and the wrong
`mouse:272 = right click` note now says `mouse:273`. `btop.conf` was
deliberately left alone: btop regenerates the whole file, comments included,
whenever it saves a setting, so a sweep would not survive.

**READMEs — games done.** `rolt/README.md`'s `Credits` and its
`status`-as-liveness paragraph are now pointers into `bjak/README.md`, which
keeps the full text. `rolt/IMPLEMENTATION.md` lost its copy of the test output
and of the IPC table (both live in the README).

The baselines for the `home/` edits were lost with the scratchpad, so those two
files were checked by reading the result rather than by the comment-stripping
comparison.

**Still outstanding:**

- Nothing left in the READMEs: `pokr`, `busride` and `bons` were read too. Only
  `bons`'s PAY BACK paragraph was duplicated (now a pointer to busride's bank
  section); `pokr` is where the "`show` is unreachable" note originates, so it
  stays.

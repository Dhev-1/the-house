...
# one). hyprland.lua holds the geometry and motion, everything with a colour in
...
# window comes from the glow underneath it, not from the border. Held as a Lua
# value, since colors.lua is where it lands: a string, or a gradient table.
rail="\"rgba(${accent}ee)\""
bordersize=2
dim=0.25
...
# hl.config calls merge, so this is a second call rather than a second file; and
# hyprctl reload below re-reads hyprland.lua first, so the next table that is
...
# --- Hyprland, live -----------------------------------------------------------
# Setting a value alone updates the stored value but doesn't redraw borders until
# the window is refocused. A reload re-runs hyprland.lua, colors.lua last, and
# repaints every border; it does not fire hyprland.start again, so autostart is
# not re-run and it is cheap and safe.
